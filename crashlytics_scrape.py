...
def get_crash_rate():
    ...
    try:
        return str(Fabric.driver.find_element_by_xpath("//div[@class='issues_metrics']/div[1]/div/span/div[2]/div/div/span").text)
    except StaleElementReferenceException as e:
...
def get_icons_if_any(crash, crash_info, link_index, crash_data_dict):
    if check_crash_for_icons(crash):
        ...
        if link:
            ...
            while loop:
                try:
                    crash_data_dict = get_crash_icons(crash, link, icon_index, crash_data_dict)
                except NoSuchElementException:
                    loop = False
                    print("[main] No more badges for crash {}".format(crash_info[0]))
                except StaleElementReferenceException:
                    print("[main] Stale element when looping for icons.")
                    loop = False
                except Exception:
                    if icon_index > 1:
                        print("[main] No more icons for crash {}".format(crash_info[0]))
                    else:
...
def get_crash_icons(crash, link, icon_index, crash_data_dict):
    ...
    if 'note' in icon_attribute:
        ...
        print("[get_crash_icons] PERCENT ROOTED = {}".format(crash_data_dict['rooted']))
        ...
        print("[get_crash_icons] DEVICE = {}".format(crash_data_dict['device']))
    elif 'Android' in icon_attribute or 'iOS' in icon_attribute:
        crash_data_dict['os_version'] = icon.text
        print("[get_crash_icons] OS VERSION = {}".format(crash_data_dict['os_version']))
    else:
        pass
    return crash_data_dict
    ...
    try:
        ...
        for bundles, versions in bundles_and_versions.items():
            for version in versions:
                print("[main] Bundle: {}, Version: {}".format(bundles, version))
                ...
                for crash in crash_table:
                    ...
                    crash_data_dict['crashes'] = crash_info[len(crash_info)- 4]
                    crash_data_dict['users'] = crash_info[len(crash_info) - 2]
                    crash_data_dict['crash_rate'] = get_crash_rate()
                    crash_data_dict['first_seen'], crash_data_dict['last_seen'] = get_first_last_seen(crash_info)
                    crash_data_dict = get_icons_if_any(crash, crash_info, link_index, crash_data_dict)
                    try:
                        crash_search = session.query(TopCrashes).filter(TopCrashes.crash_subtitle == crash_data_dict['subtitle']).filter(TopCrashes.app_version == crash_data_dict['version']).one()
